...
class Train:
    ...
    def update_position(self):
        ...
        if self.progress >= 1.0:
            self.progress = 0.0
            self.waypoint_idx = (self.waypoint_idx +
                                 1) % (len(topology['waypoints']) - 1)
    ...
    def update_telemetry(self, lat, lon):
        ...
        for curve in topology['sharp_curves']:
            dist = math.sqrt(
                (lat - curve['lat'])**2 + (lon - curve['lon'])**2) * 111  # km
...
def main():
    ...
    print(
        f"🚂 Starting simulation: {NUM_TRAINS} trains × {EVENTS_PER_SEC} events/sec")
    ...
    try:
        while True:
            for train in trains:
                ...
                try:
                    # kafka-python uses .send() and returns a
